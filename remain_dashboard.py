...
def add_product():
    ...
    label_availability = tk.Label(add_frame, font=("Segoe UI", 11), text="Availability:", pady=8)
    ...
    cmb_avail['values'] = ('Available', 'Not Available')
    cmb_avail.current(0)
    ...
    var_lsleeve = IntVar ()
    cb_lsleeve = tk.Checkbutton(add_frame, font=("Segoe UI", 10), text="Long Sleeved", variable=var_lsleeve, onvalue=1, offvalue=0)
...
def search():
    ...
    tbl_view.insert('', tk.END, values=val)
    

    #panghanap entry sa dbase
    #c.execute("SELECT * FROM Records WHERE number = ?", (searchID))
...
tbl_view.heading("prodAvail", text="Availability")
